@ECHO OFF
:: 20190708 HV
:: ETHZ_Evidence_Collector_x86_v20190708
...
:: step #06 pslist
.\tools\ETHZ_EC_pslist.exe -accepteula -nobanner -t > %outputdir%\06_pslist.txt
...
reg query "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Run" >> %outputdir%\16_machine_registry.txt 2>&1
reg query "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\RunOnce" >> %outputdir%\16_machine_registry.txt 2>&1
...
reg query "HKLM\Software\Microsoft\Windows\CurrentVersion\Uninstall" /s >> %outputdir%\16_machine_registry.txt
reg query "HKLM\Software\Microsoft\Active Setup\Installed Components" /s >> %outputdir%\16_machine_registry.txt
...
.\tools\ETHZ_EC_autorunsc.exe -accepteula -nobanner -a * -h -s -t * > %outputdir%\26_autorunsc.txt
.\tools\ETHZ_EC_autorunsc.exe -accepteula -nobanner -a * -h -s -t -c * > %outputdir%\26_autorunsc.csv
